...
export default async argv => {
  ...
  const isLoopbackHost = host => {
    ...
    let h = value
    try {
      h = new URL(value.includes('://') ? value : `http://${value}`).hostname.toLowerCase()
    } catch {
      h = value
    }
    return h === 'localhost' || h === '127.0.0.1' || h === '::1' || h.endsWith('.localhost')
  }
  const buildRemoteRequestURLs = (remoteHost, remoteResource) => [
    `https://${remoteHost}/${remoteResource}`,
    `http://${remoteHost}/${remoteResource}`,
  ]
  ...
  const remoteGet = (remote, slug, cb) => {
    const remoteURLs = buildRemoteRequestURLs(remote, `${slug}.json`)
    fetchWithProtocolFallback(remoteURLs)
      .then(async ({ response }) => {
        if (!response.ok) {
  ...
  app.get('/proxy/*splat', authorized, (req, res) => {
    ...
    const requestURLs = buildRemoteRequestURLs(remoteHost, remoteResource)
    console.log('PROXY Request: ', requestURLs[0])
    if (!remoteHost || !remotePath) {
      res.status(400).end()
      return
    }
    if (!isLoopbackHost(wikiName) && isLoopbackHost(remoteHost)) {
      res.status(403).end()
      return
    }
    if (
      ...
      fetchWithProtocolFallback(requestURLs)
