...
const fetchWithProtocolFallback = async requestURLs => {
  let lastError
...
export default async argv => {
  ...
  const remoteGet = (remote, slug, cb) => {
    ...
    fetchWithProtocolFallback(candidates)
      .then(async ({ response }) => {
        if (!response.ok) {
  ...
  app.get('/proxy/*splat', (req, res) => {
    ...
    if (
      path.extname(requestURL.pathname).toLowerCase() === '.json' ||
      path.extname(requestURL.pathname).toLowerCase() === '.png' ||
      path.extname(requestURL.pathname).toLowerCase() === '.jpg' ||
      pathParts[2] === 'plugin'
    ) {
      fetchWithProtocolFallback(candidates)
        .then(async ({ requestURL, response: fetchRes }) => {
          if (fetchRes.ok) {
