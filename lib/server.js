...
export const isLoopbackHost = host => {
  ...
  let normalizedHost = value
  try {
    normalizedHost = new URL(value.includes('://') ? value : `http://${value}`).hostname.toLowerCase()
  } catch {
...
export const buildRemoteRequestURLs = (wikiName, remoteHost, remoteResource) => {
  const blocked = !isLoopbackHost(wikiName) && isLoopbackHost(remoteHost)
  return {
    blocked,
    candidates: blocked ? [] : [`https://${remoteHost}/${remoteResource}`, `http://${remoteHost}/${remoteResource}`],
  }
}

const fetchWithProtocolFallback = async (requestURLs, { timeoutMs = 2000, headers } = {}) => {
  let lastError
...
const proxyResponseHeaderAllowlist = new Set(['content-type', 'cache-control', 'etag', 'last-modified'])
...
export default async argv => {
  ...
  if (argv.csp) {
    const connectSrc = isLoopbackHost(wikiName) ? argv.csp_connect_src_loopback : argv.csp_connect_src_public
  ...
  const remoteGet = (remote, slug, cb) => {
    const { blocked, candidates } = buildRemoteRequestURLs(wikiName, remote, `${slug}.json`)
    if (blocked) {
      cb(new Error('Loopback targets are blocked'), 'Page not found', 403)
      return
    }
    fetchWithProtocolFallback(candidates, { timeoutMs: 8000 })
      .then(async ({ response }) => {
        if (!response.ok) {
          cb(null, 'Page not found', response.status)
          return
        }
        const json = await response.json()
        cb(null, json, response.status || 200)
      })
      .catch(err => {
  ...
  app.get('/proxy/*splat', (req, res) => {
    const requestURL = new URL(req.originalUrl, 'http://local')
    const pathParts = requestURL.pathname.split('/').filter(Boolean)
    const remoteHost = pathParts[1]
    const remotePath = pathParts.slice(2).join('/')
    const remoteResource = `${remotePath}${requestURL.search}`
    const { blocked, candidates } = buildRemoteRequestURLs(wikiName, remoteHost, remoteResource)
    console.log('PROXY Request: ', candidates[0] || `blocked:${remoteHost}/${remoteResource}`)
    ...
    const isJsonRequest = extension === '.json'
    const isImageRequest = extension === '.png' || extension === '.jpg'
    const isPluginRequest = pathParts[2] === 'plugin'
    if (isJsonRequest || isImageRequest || isPluginRequest) {
      const timeoutMs = isJsonRequest || isPluginRequest ? 20000 : 2000
      fetchWithProtocolFallback(candidates, { timeoutMs })
        .then(async ({ response: fetchRes }) => {
          if (fetchRes.ok) {
            for (const [key, value] of fetchRes.headers.entries()) {
              const normalized = key.toLowerCase()
              if (proxyResponseHeaderAllowlist.has(normalized)) {
        ...
        .catch(err => {
          console.log('ERROR: Proxy Request ', candidates, err)
          if (!res.headersSent) {
            res.status(502).end()
          } else {
            res.destroy(err)
